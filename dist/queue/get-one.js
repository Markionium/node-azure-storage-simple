    ...
    return new Promise(function (resolve, reject) {
      try {
        svc.getMessages(name, 1, function (err, result, response) {
          ...
          if (!(result && result.length)) return resolve(null); //unbox result array          

          result = result[0]; //inherit wrapped object from original result

          var ret = Object.create(result); //parse result value
